  ...
  static userLogin = async (req: any, res: any) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .send(helper.ResponseData(400, "Email is incorrect", null, null));
      }
      const comparePassword = await bcrypt.compare(password, user.password);
      if (!comparePassword) {
        return res
          .status(400)
          .send(helper.ResponseData(400, "Password incorrect", error, null));
      }
      const dataUser = {
      ...
      return res
        .status(400)
        .send(helper.ResponseData(400, "Server Error", error, null));
    }
  };
}
